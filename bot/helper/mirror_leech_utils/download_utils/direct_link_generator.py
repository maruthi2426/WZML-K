...
def hubcloud_handler(link):
    ...
    try:
        ...
        if isinstance(data, dict) and "error" not in data:
            ...
            raise DirectDownloadLinkException(f"ERROR: HubCloud bypass failed - {data['error']}")
...
def direct_link_generator(link, quality_filter=None):
    ...
    elif any(x in domain for x in ["gdflix.net", "gdflix.cc", "gdflix.ml"]) or "gdflix" in domain:
        return gdflix(link)
    elif any(x in domain for x in ["hubcloud.foo", "hubcloud.one", "hubcloud.cc", "hubcloud.net"]) or "hubcloud" in domain:
